...
排查会有1定的帮助。为此，虚拟机提供了1些跟踪系统状态的参数，使用给定的参数
执行 Java 虚拟机，就可以在系统运行时打印相关日志，用于分析实际问题。我们进行
...
-Xmn: 可以设置新生代的大小，设置1个比较大的新生代会减少老年代的大小，这个参数对系统性能
以及GC行为有很大的影响，新生代大小1般会设置整个堆空间的 1/4 到 1/3 左右。
-XX:SurvivorRatio:用来设置新生代中 eden 空间的 from/to 空间的比例。含义:
-XX:SurvivorRatio=eden/from=eden/to
总结:
    在实际工作中，我们可以直接将初始的堆大小与最大堆大小设置相等，这样的好处是
可以减少程序运行时的垃圾回收次数，从而提高性能。
    不同的堆分布情况，对系统执行会产生1定的影响，在实际工作中，应该根据系统的特点做出合理
的配置，基本策略:尽可能将对象预留在新生代，减少老年代的GC次数。
