...
--------------------------
局部变量表:
    存放了编译器可知的各种基本数据类型（boolean,byte,char,short,int,float,long,double）、对象引用（reference类型，不同于对象本身， 
可能是1个指向对象起始地址的引用指针，也可能是指向1个代表对象的句柄或其他与此对象相关的位置）和 returnAddressleixing (字节码指令地址）。局部变量表所需内存在编译期间完成分配，运行期间不会改变。
...
---------------------------------------------------
5:Java 堆:
    Java 堆是整个虚拟机所管理的最大内存区域，所有的对象创建都是在这个区域进行内存分配，是被所有线程共享的1块内存区域，在虚拟机启动时创建，
此内存区域的唯1目的就是存放对象实例，几乎所有的对象实例以及数组都在这里分配内存。
Java 堆是垃圾收集器管理的主要区域（方法区也需要回收），因此又称为 GC 堆(Garbage Collected Heap)。
现在收集器基本采用分代收集算法，可以将堆分为新生代和老年代。划分的好处是可以方便垃圾的准确回收。

    Java 堆可以处于物理上不连续的内存空间中，只要逻辑上是连续的即可。堆还可以动态增加其内存，当堆中无法申请到新内存创建实例，并且堆也无法再扩展时，
将会抛出 OutOfMemroyError。
...
---------------------------------------------------
6:方法区:
    方法区与 Java 堆1样，是各个线程共享的内存区域，它用于存储已被虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码等数据。方法区也是所有线程共享。
方法区逻辑上属于堆的1部分，但是为了与堆进行区分，通常又叫“非堆”。
方法区和堆1样不需要连续的内存，并且可以动态扩展，动态扩展失败1样会抛出 OutOfMemoryError 异常。

    在 HotSpot 虚拟机中，把方法区当做永久代来进行GC，对其回收的目标主要是针对常量池的回收以及对类型的卸载，但是1般比较难实现。
垃圾收集行为在这个区域是比较少出现的，
但并非数据进入方法区后就“永久存在”了。由于方法区主要存储类的相关信息，所以对于动态生成类的情况比较容易出现永久代的内存溢出。
...
7:运行时常量池:
    ...
    既然运行时常量池是方法区的1部分，自然受到方法区内存的限制，当常量池无法再申请到内存时会抛出 OutOfMemoryError 异常。JDK1.7及之后版本的 JVM 
...
JDK1.4 中新加入的 NIO(New Input/Output) 类，引入了1种基于通道（Channel） 与缓存区（Buffer） 的 I/O 方式，它可以直接使用 Native 函数库直接分配堆外内存，
然后通过1个存储在 Java 堆中的 DirectByteBuffer 对象作为这块内存的引用进行操作。这样就能在1些场景中显著提高性能，因为避免了在 Java 堆和 Native  
堆之间来回复制数据。
